...
# spec/decidim_dummy_app é versionada no repositório (gerada localmente via
# `rake test_app`), então a imagem NÃO precisa de PostgreSQL/Redis no build.
# O banco é criado/migrado no start do container (db:prepare no CMD) contra
# os serviços externos do ambiente (compose/Kubernetes).
