...
image:https://qlty.sh/gh/decidim/projects/decidim/maintainability.svg[Maintainability,link=https://qlty.sh/gh/decidim/projects/decidim]
...
There are two supported ways to run this repository, each with its own approach:

* *Local development* — Docker Compose (`docker-compose.yml` at the root).
* *Staging deploy* — Helm chart + Skaffold (`deploy/` and root `skaffold.yaml`).
...
This builds the image from the monorepo source (development mode: all gems included, no asset precompilation), starts PostgreSQL 16 and Redis, and runs database migrations plus Decidim seeds on first boot. The app is available at http://localhost:3000 (seeded admin: `admin@example.org` / `decidim123456789`).
...
=== Staging deploy (Helm + Skaffold)

For a staging environment on Kubernetes:
...
This builds the production image (assets precompiled) and installs the Helm chart (`deploy/helm/decidim`) into the `decidim-staging` namespace, provisioning PostgreSQL (CloudNativePG), Redis and a persistent volume for uploads. Migrations run automatically on container boot (`db:prepare`).

Prerequisites: Docker with BuildKit, Skaffold v2+, kubectl, Helm v3+ and a CloudNativePG operator installed in the target cluster.
See `deploy/README.md` for the full reference (external database, SMTP, troubleshooting).
